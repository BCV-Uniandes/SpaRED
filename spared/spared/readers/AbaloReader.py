...
class AbaloReader():
    def __init__(self,
        dataset: str = 'abalo_human_squamous_cell_carcinoma',
        param_dict: dict = {
            'cell_min_counts':   500,
    ...
    def get_split_names(self) -> dict:
        ...
        train_data = {
            'abalo_human_squamous_cell_carcinoma':          ['V10F24-015_A1', 'V10F24-015_C1'],
            }
        
        val_data = {
            'abalo_human_squamous_cell_carcinoma':          ['V10F24-015_B1'],
            }
        
        test_data = {
            'abalo_human_squamous_cell_carcinoma':          ['V10F24-015_D1'],
            }
    ...
    def download_data(self) -> str:
        ...
        if not os.path.exists(os.path.join(SPARED_PATH, 'processed_data', f'{dataset_author}_data', self.dataset)) or self.force_compute:
            ...
            wget.download('https://prod-dcd-datasets-cache-zipfiles.s3.eu-west-1.amazonaws.com/2bh5fchcv6-1.zip', out=os.path.join(SPARED_PATH, 'data', f'{dataset_author}_data','zipped_data.zip'))
            ...
            with zipfile.ZipFile(os.path.join(SPARED_PATH, 'data', f'{dataset_author}_data', 'zipped_data.zip'), 'r') as zip_ref:
                zip_ref.extractall(os.path.join(SPARED_PATH, 'data', f'{dataset_author}_data', self.dataset))
